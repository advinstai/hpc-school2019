...
static void force(mdsys_t *sys) 
{
    ...
    epot=0.0;
    memset(sys->cx,0,sizeof(double)*natoms*sys->nthreads);
    memset(sys->cy,0,sizeof(double)*natoms*sys->nthreads);
    memset(sys->cz,0,sizeof(double)*natoms*sys->nthreads);
...
#pragma omp parallel for private(i) reduction(+:epot)
        ...
        for(i=rank; i < natoms-1; i += size) {
            ...
            int tid = 0;
#endif
            fx = sys->cx + tid*natoms;
            fy = sys->cy + tid*natoms;
            fz = sys->cz + tid*natoms;
...
#pragma omp parallel for private(i)
    ...
    for (i=0; i < natoms; ++i) {
        for (int j = 1; j < sys->nthreads; ++j) {
            sys->cx[i] += sys->cx[i+j*natoms];
            sys->cy[i] += sys->cy[i+j*natoms];
            sys->cz[i] += sys->cz[i+j*natoms];
        }
    }
