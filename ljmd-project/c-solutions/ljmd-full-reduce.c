 ...
 *
 * cell list c version with MPI and threads.
 */
...
{   
    ...
    const double *vel = sys->vel;
#if defined(_OPENMP)
#pragma omp parallel for private(i) reduction(+:ekin)
#endif 
...
static void force(mdsys_t *sys) 
{
    ...
    MPI_Bcast(sys->pos,3*natoms,MPI_DOUBLE,0,MPI_COMM_WORLD);
    epot = 0.0;
    memset(sys->tmp,0,sizeof(double)*3*sys->natoms*sys->nthreads);
...
#pragma omp parallel reduction(+:epot)
#endif
    {
        ...
        tid=0;
#endif
        fx=sys->tmp + (3*tid*natoms);
        fy=sys->tmp + ((3*tid+1)*natoms);
        fz=sys->tmp + ((3*tid+2)*natoms);
        rx=sys->pos;
        ry=sys->pos + natoms;
        rz=sys->pos + 2*natoms;

        /* self interaction of atoms in cell */
        for(i=rank*sys->nthreads; i < sys->ncell; i += size*sys->nthreads) {
            int j;
            const cell_t *c1;
        ...
        for(i=rank*sys->nthreads; i < sys->npair; i += size*sys->nthreads) {
            int j;
            const cell_t *c1, *c2;
...
#pragma omp barrier
        ...
        for (i=1; i < sys->nthreads; ++i) {
            ...
            for (j=fromidx; j < toidx; ++j) {
                sys->tmp[j] += sys->tmp[offs+j];
            }
        }
    }
    MPI_Reduce(&epot,&sys->epot,1,MPI_DOUBLE,MPI_SUM,0,MPI_COMM_WORLD);
    MPI_Reduce(sys->tmp,sys->frc,3*natoms,MPI_DOUBLE,MPI_SUM,0,MPI_COMM_WORLD);
 }
...
int main(int argc, char **argv) 
{
    ...
    sys.tmp=(double *)malloc(sys.nthreads*3*sys.natoms*sizeof(double));
    ...
    if (rank == 0) {
        ...
        if(fp) {
            ...
            fclose(fp);
            azzero(sys.tmp, 3*sys.nthreads*sys.natoms);
        } else {
    ...
    MPI_Bcast(sys.pos,3*sys.natoms,MPI_DOUBLE,0,MPI_COMM_WORLD);
    MPI_Bcast(sys.vel,3*sys.natoms,MPI_DOUBLE,0,MPI_COMM_WORLD);
    memset(sys.frc,0,3*sizeof(double)*sys.natoms);
    memset(sys.tmp,0,sys.nthreads*3*sizeof(double)*sys.natoms);
